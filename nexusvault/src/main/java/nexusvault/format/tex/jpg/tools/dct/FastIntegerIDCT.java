...
public final class FastIntegerIDCT {
	 ...
	 *            matrix start
	 */
	public static void idct(int[] data, int offset, int rowStride) {
		for (int i = 0; i < 8; ++i) {
			idctRow(data, offset + rowStride * i);
		}

		for (int i = 0; i < 8; ++i) {
			idctColumn(data, offset + i, rowStride);
		}
	}
	...
	private static void idctColumn(int[] data, int i, int rowStride) {
		int x0, x1, x2, x3, x4, x5, x6, x7, x8;
		...
		final boolean useShortCut = (x1 | x2 | x3 | x4 | x5 | x6 | x7) == 0;
		if (useShortCut) {
			data[i + rowStride * 0] = data[i + rowStride * 1] = data[i + rowStride * 2] = data[i + rowStride * 3] = data[i + rowStride * 4] = data[i
					+ rowStride * 5] = data[i + rowStride * 6] = data[i + rowStride * 7] = iclp[iclp_offset + (data[i + rowStride * 0] + 32 >> 6)];
			return;
		}
